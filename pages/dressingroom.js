...
function Dressingroom() {
    ...
    async function Newapi() {
        // let newApi= await fetch("https://stg.knowlytes.com/api/api/getfreezetraits")
    ...
    const renderTokensForOwner = async (owenrAddress) => {
        ...
        try {
            if (owenrAddress) {
                ...
                let response = await fetch(`https://stg.knowlytes.com/api/gettokenid/${owenrAddress1}`)
                // let response = await fetch(`https://stg.knowlytes.com/api/api/gettokenid/${owenrAddress1}`)
                ...
                for (let i = 0; i < response1.data.length; i++) {
                    ...
                    if(metaData == ""){
                        const dummydata = {image:`https://stg.knowlytes.com/eventapi/dknowlyets/0.gif`, name:`Knowlytes #${response1.data[i].tokenId}`}
                        UserNftName.push(dummydata.name);
                        url.push(dummydata.image);
                        allData.push(dummydata)
                    }else{
